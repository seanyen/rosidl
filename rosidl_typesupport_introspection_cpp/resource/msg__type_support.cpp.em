...
def is_vector_bool(member):
    from rosidl_parser.definition import BasicType
    from rosidl_parser.definition import Sequence
    return isinstance(member.type, Sequence) and isinstance(member.type.basetype, BasicType) and member.type.basetype.type == 'boolean'
}@
@# exclude std::vector<bool> because of specialization in their API
@[  if isinstance(member.type, NestedType) and not is_vector_bool(member)]@
@{
from rosidl_generator_cpp import  MSG_TYPE_TO_CPP
if isinstance(member.type.basetype, BasicType):
    type_ = MSG_TYPE_TO_CPP[member.type.basetype.type]
elif isinstance(member.type.basetype, String):
    type_ = 'std::string'
elif isinstance(member.type.basetype, WString):
    assert False, 'Unknown type: ' + str(member.type.basetype)
elif isinstance(member.type.basetype, NamespacedType):
    type_ = '::'.join(member.type.basetype.namespaces + [member.type.basetype.name])
}@
size_t size_function__@(message.structure.type.name)__@(member.name)(const void * untyped_member)
{
...
const void * get_const_function__@(message.structure.type.name)__@(member.name)(const void * untyped_member, size_t index)
{
@[    if isinstance(member.type, Array)]@
  const auto & member =
...
void * get_function__@(message.structure.type.name)__@(member.name)(void * untyped_member, size_t index)
{
@[    if isinstance(member.type, Array)]@
  auto & member =
...
void resize_function__@(message.structure.type.name)__@(member.name)(void * untyped_member, size_t size)
{
  auto * member =
    reinterpret_cast<std::vector<@(type_)> *>(untyped_member);
  member->resize(size);
...
@[  end if]@
@[end for]@
static const ::rosidl_typesupport_introspection_cpp::MessageMember @(message.structure.type.name)_message_member_array[@(len(message.structure.members))] = {
@{
for index, member in enumerate(message.structure.members):
    ...
    if isinstance(type_, BasicType):
        # uint8_t type_id_
        print('    ::rosidl_typesupport_introspection_cpp::ROS_TYPE_%s,  // type' % type_.type.upper())
    ...
    print('    offsetof(%s::%s, %s),  // bytes offset in struct' % ('::'.join([package_name] + list(interface_path.parents[0].parts)), message.structure.type.name, member.name))
    # void * default_value_
    print('    nullptr,  // default value')  # TODO default value to be set

    function_suffix = ('%s__%s' % (message.structure.type.name, member.name)) if isinstance(member.type, NestedType) and isinstance(member.type.basetype, NamespacedType) else None
    ...
    print('    %s  // resize(index) function pointer' % ('resize_function__%s' % function_suffix if function_suffix and isinstance(member.type.basetype, Sequence) else 'NULL'))
...
static const ::rosidl_typesupport_introspection_cpp::MessageMembers @(message.structure.type.name)_message_members = {
  ...
  sizeof(@('::'.join([package_name] + list(interface_path.parents[0].parts) + [message.structure.type.name]))),
  @(message.structure.type.name)_message_member_array  // message members
};

static const rosidl_message_type_support_t @(message.structure.type.name)_message_type_support_handle = {
  ::rosidl_typesupport_introspection_cpp::typesupport_identifier,
  &@(message.structure.type.name)_message_members,
...
{
  return &::@('::'.join([package_name] + list(interface_path.parents[0].parts)))::rosidl_typesupport_introspection_cpp::@(message.structure.type.name)_message_type_support_handle;
}
